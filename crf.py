...
class CRF(nn.Module):
    ...
    def __init__(self, num_tags: int, batch_first: bool = False) -> None:
        ...
        self.reset_parameters() #对上面三个参数都重新初始化
    ...
    def forward(self, emissions: torch.Tensor,
        ...
        # emissions:(batch,seqlen,num_tags),tags:(batch,seqlen),mask:(batch,seqlen)
        if self.batch_first:  # true
            emissions = emissions.transpose(0, 1) # ->(seqlen,batch,num_tags)
            tags = tags.transpose(0, 1) # ->tags:(seqlen,batch)
            mask = mask.transpose(0, 1) # ->mask:(seqlen,batch)
    ...
    def _compute_score(self, emissions: torch.Tensor,
        ...
        seq_length, batch_size = tags.shape #290,batch
        mask = mask.float()
    ...
    def _compute_normalizer(self, emissions: torch.Tensor,
        ...
        for i in range(1, seq_length):
            ...
            # and emitting
            # --> (batch_size, num_tags, num_tags)=(b,num_tags,1)+(num_tags,num_tags)+(b,1,num_tags)
